...
*/
    ...
    // });
    // 
    deleteone(divisionIdRef: number , empLastNameIdRef: number , empFirstNameIdRef: number , empSecondNameIdRef: number , employeeId: number ): Observable<ILprEmployee02View> {
        let params: HttpParams  = new HttpParams({encoder: this.appGlblSettings});
        let hasFilter: boolean = false;
        if (!(typeof employeeId === 'undefined')) {
            if (!(employeeId === null)) {
